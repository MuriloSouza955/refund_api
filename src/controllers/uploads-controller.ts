...
import { DiscStorage } from "@/providers/disk-storage";
import uploadConfig from "@/configs/upload";
import { AppError } from "@/utils/AppError";

class UploadsController {
  async create(request: Request, response: Response) {
    const diskStorage = new DiscStorage();
    try {
      const fileSchema = z
        .object({
          ...
          mimetype: z
            ...
            .refine(
              ...
              `Formato de arquivo invalido. Formatos permitidos: ${uploadConfig.ACCEPTED_IMAGE_TYPES}`
            ),
          size: z
            .number()
            .positive()
